...
> = async ({ queryKey }) => {
  try {
    ...
    const url = `${apiProtocol}://${apiHost}/staff/orders?page=${page}&limit=${limit}&status=${status}`;
...
> = async ({ queryKey }) => {
  try {
    const [, id] = queryKey;
    const url = `http://localhost:3020/staff/orders/${id}`;
...
> = async ({ queryKey }) => {
  try {
    ...
    const url = `http://localhost:3020/staff/orders/${id}/items?page=${page}&limit=${limit}`;
